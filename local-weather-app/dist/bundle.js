...
var _browser = __webpack_require__(128);
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            weather = window.sessionStorage.darkSky;
            address = window.sessionStorage.googleMaps;
            ...
            _context.prev = 3;
            _context.next = 6;
            return (0, _browser.checkNavigator)().then(function (pos) {
              return (0, _api.callApi)((0, _api.googleMapsUrlBuilder)(pos), 'Google Maps');
            });

          case 6:
            window.sessionStorage.googleMaps = _context.sent;
            _context.next = 9;
            return (0, _browser.checkNavigator)().then(function (pos) {
              return (0, _api.callApi)((0, _api.darkSkyUrlBuilder)(pos), 'Dark Sky');
            });

          case 9:
            window.sessionStorage.darkSky = _context.sent;
          ...
          case 14:
            ...
            weather = _api.dummyData;
            address = 'Seymour, CT, USA';

          case 20:
            (0, _browser.writeToDoc)(weather);
            _context.next = 25;
            break;

          case 23:
            console.log('Using data from session storage.');
            (0, _browser.writeToDoc)(JSON.parse(weather), JSON.parse(address));
...
exports.callApi = exports.googleMapsUrlBuilder = exports.darkSkyUrlBuilder = exports.dummyData = exports.apiLangs = undefined;

var _langSearch = __webpack_require__(129);

var _langSearch2 = _interopRequireDefault(_langSearch);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var apiLangs = ['ar', 'az', 'be', 'bg', 'bs', 'ca', 'cs', 'de', 'el', 'en', 'es', 'et', 'fr', 'hr', 'hu', 'id', 'is', 'it', 'kw', 'nb', 'nl', 'pl', 'pt', 'ru', 'sk', 'sl', 'sr', 'sv', 'tet', 'tr', 'uk', 'x-pig-latin', 'zh', 'zh-tw']; /* eslint-env browser */
...
function darkSkyUrlBuilder(position) {
  var browserBits = urlParts(position);
  var lang = (0, _langSearch2.default)(apiLangs, browserBits.lang);
  var key = '767b3baa2aca876fa6ea5e4fbd75228c';
  var cors = 'https://cors-anywhere.herokuapp.com/';
  var darkSky = 'https://api.darksky.net/forecast/';
  var queries = '?exclude=minutely,hourly,daily&lang=' + lang + '&units=auto';
  var url = '' + cors + darkSky + key + '/' + browserBits.loc + queries;
  return url;
}
...
function googleMapsUrlBuilder(position) {
  var browserBits = urlParts(position);
  // const key = ''
  var googleMaps = 'https://maps.googleapis.com/maps/api/geocode/json';
  var parameters = '?latlng=' + browserBits.loc + '&language=' + browserBits.browserLang;
  var url = '' + googleMaps + parameters;
  return url;
}
...
function callApi(url, service) {
  return new Promise(function promiseResponse(resolve, reject) {
    ...
    request.onloadstart = function (_) {
      return console.log(service + ': LOAD START');
    };
    ...
    request.onerror = function (_) {
      return reject(Error(service + ': Network error on getting data.'));
    };
    request.onloadend = function (_) {
      return console.log(service + ': LOAD END');
    };
    request.send();
  });
}

exports.apiLangs = apiLangs;
exports.dummyData = dummyData;
exports.darkSkyUrlBuilder = darkSkyUrlBuilder;
exports.googleMapsUrlBuilder = googleMapsUrlBuilder;
...
function checkNavigator() {
  ...
  return new Promise(function promiseLocation(resolve, reject) {
    var geoOptions = { enableHighAccuracy: true, timeout: 15000, maximumAge: 900000 };
...
function writeToDoc(weather, address) {
  var elm = function elm(element) {
    return document.querySelector(element);
  };
  // const coordinates = `${weather.latitude}, ${weather.longitude}`
  var addressString = address.results[1].formatted_address;
  var current = weather.currently;
  var time = new Date(current.time * 1e3);
...
exports.checkNavigator = checkNavigator;
exports.writeToDoc = writeToDoc;
...
function binarySearch(arr, val) {
  var exactMatch = exact(arr, val);
  var fuzzyMatch = fuzzy(arr, val);

  // return search results
  if (exactMatch) return [2];else if (fuzzyMatch[0]) return [1, fuzzyMatch[1]];else return [0];
}
...
function fuzzy(arr, val) {
  ...
  var highIndex = arr.length - 1;
  var midIndex = Math.floor((lowIndex + highIndex) / 2);
  var match = val.indexOf(arr[midIndex]) === 0;
...
function setLanguage(arr, lang) {
  var results = binarySearch(arr, lang);
  try {
    switch (results[0]) {
      case 2:
...
var $export = __webpack_require__(0);
var $re = __webpack_require__(136)(/[\\^$*+?.()|[\]{}]/g, '\\$&');
...
// 20.3.4.36 / 15.9.5.43 Date.prototype.toISOString()
var $export = __webpack_require__(0);
var toISOString = __webpack_require__(133);
...
if (!(TO_PRIMITIVE in proto)) __webpack_require__(12)(proto, TO_PRIMITIVE, __webpack_require__(134));
